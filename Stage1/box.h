...
struct Box
{
    ...
    */
    ...
    // απανταει αν συνεβει το collision η οχι
    ...
    */
    ...
    // απαντα αν λογο της βαρυτητας μπηκε το box μου μεσα σε ενα αλλο
    // αν ναι τοτε επιστρεφει ποσο πρεπει να μετακινηθει το box μου για να μην υπαρχει collision
    ...
    */
    ...
    /** Detects an intersection when this Box is "below" (larger y values) a target box (other)
*   and reports the adjustment offset so that the two boxes are separated.
*
*   \param other is the second (target) Box to test collision on the vertical axis with.
*   \return the offset this Box needs to move in the y axis, so that it does not collide
